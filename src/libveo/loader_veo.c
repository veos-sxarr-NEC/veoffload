...
*/
int init_stack_veo(veos_handle *handle, int argc,
		   char **argv, char **envp,
		   struct ve_start_ve_req_cmd *start_ve_req)
{
	...
	*areap_8b++ = (long long)NULL;
	...
	for (auxv_type = 1; auxv_type < 32; auxv_type++) {
		...
		case AT_RANDOM:
			...
			*areap_8b++ = auxv_type;
			*areap_8b++ = (long long)ve_string_addr;
			memcpy(areap_1b, (void *)auxv_val, RANDOM_BYTE);
